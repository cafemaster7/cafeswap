...
const CafeMaker = artifacts.require('CafeMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('CafeMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.cafe = await CafeToken.new({ from: alice });
        await this.cafe.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await CafeMaker.new(this.factory.address, bar, this.cafe.address, this.weth.address);
        this.cafeWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.cafe.address)).logs[0].args.pair);
    ...
    it('should make CAFEs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.cafeWETH.address, '10000000', { from: minter });
        await this.cafe.transfer(this.cafeWETH.address, '10000000', { from: minter });
        await this.cafeWETH.mint(minter);
        ...
        // After calling convert, bar should have CAFE value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.cafe.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for CAFE-ETH pair
        await this.cafe.transfer(this.cafeWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.cafeWETH.address, '100000', { from: minter });
        await this.cafeWETH.sync();
        await this.cafe.transfer(this.cafeWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.cafeWETH.address, '10000000', { from: minter });
        await this.cafeWETH.mint(minter);
        assert.equal((await this.cafeWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.cafe.address, this.weth.address);
        assert.equal((await this.cafe.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.cafeWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});
